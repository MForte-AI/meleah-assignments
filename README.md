...
## Step 8 — Run the Story Studio Database Update

The Story Studio v2 feature (vocabulary cards + 3-session FaceTime structure) needs one small addition to the database. You only do this **once**.

1. Go to **[supabase.com](https://supabase.com)** and sign in
2. Click **SQL Editor** → **New query**
3. Open `supabase/migrations/003_story_studio_v2.sql` in TextEdit
4. Copy everything and paste into the SQL Editor box
5. Click the green **Run** button
6. You should see "Success" — done!

> This adds one new column to store vocabulary card details. It won't change or delete any of Meleah's existing data.
...
8. Fill in the **Story Studio** section — AI generates all 3 sessions automatically (vocabulary cards, Aunt's story script, Meleah's retelling prompts)
...
Interactive activities (Tap Word, Drag Sort, Spell It, Read & Answer, Build the Word!, Rapid Word Drill) automatically add coins to Meleah's balance when she finishes — no action needed from you!
...
├── FCL Concepts/                ← Drop teacher's FCL concept docs here as you receive them
...
│   ├── blend-build.html         ← Build the Word! game (Monday iPad activity)
│   ├── rapid-drill.html         ← Rapid Word Drill game (Wednesday iPad activity)
│   └── vocab-cards.html         ← Vocabulary Cards viewer (Aunt opens on her phone for FaceTime)
...
├── .claude/
│   └── commands/
│       └── lesson-plan.md       ← Claude's reference guide for generating Meleah's lesson plans
└── supabase/
    └── migrations/
        ├── 001_initial_schema.sql       ← Original database setup (run once — Step 1)
        ├── 002_weekly_assignments.sql   ← Weekly plans database update (run once — Step 6)
        └── 003_story_studio_v2.sql      ← Story Studio v2 database update (run once — Step 8)
```
